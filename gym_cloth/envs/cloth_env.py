...
# Ryan: increase coverage thresholds to increase mean episode length for vis-mpc data
_REWARD_THRESHOLDS = {
    'coverage': 0.92, 
    'coverage-delta': 0.92,
    'height': 0.85,
    ...
    'variance-delta': 2,
    'folding-number': 0,
    'ssim': 0.75, # min structural similarity to goal image or demo
    'L2': 4000 # max L2 distance to goal image or demo. Tune this value
}
...
class ClothEnv(gym.Env):
    ...
    def __init__(self, cfg_file, subrank=None, start_state_path=None):
        ...
        if self._obs_type == '1d':
            ...
            self._hd = 56
            self._wd = 56 # Ryan: 224x224 is unnecessarily large for visual dynamics, reduce size by factor of 4
    ...
    def step(self, action, initialize=False):
        ...
        # Currently maxes out the force grip at a threshold to allow no-ops, TODO implement equivalent on real
    ...
    def _reward(self, action, exit_early, state):
        ...
        def compute_similarity(state, goal_img=self.goal_img):
            #return compare_ssim(state, self.goal_img, multichannel=True) # SSIM
            return L2(np.array([state]), goal_img)
            #return np.linalg.norm(state[7:-7,7:-7,:3] - goal_img[7:-7,7:-7,:3]) # L2
        ...
        if self.cfg['env']['goal_demo']:
            self._current_similarity = compute_similarity(state, self.goal_demo[self.goal_demo_idx])
        elif self.cfg['env']['goal_img']:
            self._current_similarity = compute_similarity(state)
        elif self._current_coverage > _REWARD_THRESHOLDS['coverage']:
        ...
        if self.cfg['env']['goal_demo'] or self.cfg['env']['goal_img']:
            log.debug("L2 {:.2f}".format(self._current_similarity))
            #z = vae.encode_two(VAEmodel, self.goal_img, state)
            #log.debug("VAE COST {:.2f}".format(np.linalg.norm(z[0]-z[1])))
    ...
    def _terminal(self):
        ...
        # Ryan: disabling coverage termination condition for oracle-wrinkle data collection.
        assert 'coverage' in self.reward_type
        rew_thresh = _REWARD_THRESHOLDS[self.reward_type]
        # Ryan: Generalize beyond smoothing - implement goal images and subgoals from demos
        if self.cfg['env']['goal_demo']:
            if self._current_similarity < _REWARD_THRESHOLDS['L2']:
                if self.goal_demo_idx == len(self.goal_demo) - 1: # last image in demo
                    self.logger.info("Cloth is sufficiently similar, {:.3f} below "
                        "threshold {:.3f} hence done".format(self._current_similarity, _REWARD_THRESHOLDS['L2']))
                    self.goal_demo_idx = 0
                    done = True
                else:
                    self.goal_demo_idx += self.goal_demo_stride # advance subgoal index
                    self.goal_demo_idx = min(self.goal_demo_idx, len(self.goal_demo) - 1)
                    self.logger.info("Moving to next subgoal...")
                self.mpc.set_cost_function(lambda traj: L2(traj, self.goal_demo[self.goal_demo_idx]))
        elif self.cfg['env']['goal_img'] and 'smooth' not in self.cfg['env']['goal_img']: # for smoothing, we can use coverage
            if self._current_similarity < _REWARD_THRESHOLDS['L2']:
                self.logger.info("Cloth is sufficiently similar, {:.3f} below "
                    "threshold {:.3f} hence done".format(self._current_similarity, _REWARD_THRESHOLDS['L2']))
                done = True
            ...
            if self._current_coverage > rew_thresh:
                ...
                #done = True # TODO Uncomment if smoothing.
    ...
    def _reset_actions(self):
        ...
        if self._init_type == 'tier0':
            logger.debug("Tier 0, so do nothing and start smooth.")

        elif self._init_type == 'tier1':
            # ------------------------------------------------------------------
    ...
    def get_random_action(self, edge_bias=False, atype='over_xy_plane'):
        """Retrieves random action.

        edge_bias: If True, sample a corner 30% of the time, non-corner edge 60%, and random 10%.
        ...
        if edge_bias:
            # assuming 25x25 grid
            assert atype == 'over_xy_plane'
            corners = [26, 48, 576, 598]
            # top + right + left + bottom
            edges = list(range(25)) + list(range(25,600,25)) + list(range(49,624,25)) + list(range(600,625))
            r = np.random.rand()
            if r < 0.4:
                # pick a corner
                idx = np.random.choice(corners)
                pt = self.cloth.pts[idx]
                x, y = pt.x, pt.y
            elif r < 0.8:
                # pick an edge
                idx = np.random.choice(edges)
                pt = self.cloth.pts[idx]
                x, y = pt.x, pt.y
            else:
                # random (but within bounding box of the cloth)
                min_x, max_x = min(self.cloth.pts, key=lambda pt: pt.x).x, max(self.cloth.pts, key=lambda pt: pt.x).x
                min_y, max_y = min(self.cloth.pts, key=lambda pt: pt.y).y, max(self.cloth.pts, key=lambda pt: pt.y).y
                while True:
                    x, y, dx, dy = self.action_space.sample()
                    if self._clip_act_space:
                        x, y = (x + 1) / 2, (y + 1) / 2
            ...
            cx = (x - 0.5) * 2.0
            cy = (y - 0.5) * 2.0
            dx, dy = self.action_space.sample()[2:]
            if np.random.rand() < 0.5: # wp 0.5, flip dx/dy if going out of bounds
                if x + dx > self.bounds[0] + self._slack/2 or x + dx < -self._slack/2:
                    dx = -dx
                if y + dy > self.bounds[1] + self._slack/2 or y + dy < -self._slack/2:
                    dy = -dy
            # truncate actions at the bounds
            if x + dx > self.bounds[0] + self._slack/2:
                dx = self.bounds[0] - x
            elif x + dx < -self._slack/2:
                dx = -x
            if y + dy > self.bounds[1] + self._slack/2:
                dy = self.bounds[1] - y
            elif y + dy < -self._slack/2:
                dy = -y
            return cx, cy, dx, dy
        if atype == 'over_xy_plane':
            # slightly optimized random - resample until picked point is within the bounding box of the cloth
            min_x, max_x = min(self.cloth.pts, key=lambda pt: pt.x).x, max(self.cloth.pts, key=lambda pt: pt.x).x
            min_y, max_y = min(self.cloth.pts, key=lambda pt: pt.y).y, max(self.cloth.pts, key=lambda pt: pt.y).y
            while True:
                cx, cy, dx, dy = self.action_space.sample()
                if self._clip_act_space:
                    x, y = (cx + 1) / 2, (cy + 1) / 2
                else:
                    x, y = cx, cy
                # flip dx or dy wp 0.5 if clearly going out of bounds
                if np.random.rand() < 0.5:
                    if x + dx > self.bounds[0] + self._slack/2 or x + dx < -self._slack/2:
                        dx = -dx
                    if y + dy > self.bounds[1] + self._slack/2 or y + dy < -self._slack/2:
                        dy = -dy
                # truncate actions at bounds
                if x + dx > self.bounds[0] + self._slack/2:
                    dx = self.bounds[0] - x
                elif x + dx < -self._slack/2:
                    dx = -x
