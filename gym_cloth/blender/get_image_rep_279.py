...
def set_camera_pose(dim_height, dim_width, camera_pose, camera_deg):
    ...
    if ADD_DOM_RAND:
        ...
        cp = [rn(0.0, scale=EPS), rn(0.0, scale=EPS), rn(0.0, scale=EPS)]
        cd = [rn(0.0, scale=EPS), rn(0.0, scale=EPS), rn(0.0, scale=EPS)]
...
def set_cloth_color(
    mesh_name, color_noise, specular_max, init_type, init_side, backfacing=False
):
    """Set the cloth color.
    ...
    bpy.context.active_object.data.materials.append(
        bpy.data.materials["cloth_material"]
    )
    ...
    if backfacing:
        ...
        if ADD_DOM_RAND:
            b = np.array([0.100, 0.350, 0.850]) + n1
            f = np.array([0.085, 0.200, 0.730]) + n1
        else:
            b = np.array([0.100, 0.350, 0.850])
            f = np.array([0.085, 0.200, 0.730])
...
def setup_scene(args):
    ...
    set_dom_rand(args["use_dom_rand"])
    ...
    # Set the camera pose
    set_camera_pose(
        args["dim_height"], args["dim_width"], args["camera_pos"], args["camera_deg"]
    )
    ...
    bed_mesh_name = load_mesh(args["bed_obj_path"])
    set_bed_color(bed_mesh_name, args["color"])
    set_bed_pose(bed_mesh_name)


def render_full_scene(obj_path, args):
    """Sets up the cloth and renders the full scene to an image"""
...
def main(args):
    ...
    setup_scene(args)
    for obj_path in args["obj_paths"].split(","):
        render_full_scene(obj_path, args)
